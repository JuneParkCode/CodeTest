...
static int width,height,global_counts;
void dp(int x, int y,int prev_color,int vIdx,vector<vector<int>> &picture){
    if(x>=width || y>=height ||x<0 || y<0|| searchedPixel[y][x]){
        return;
    }
    if(prev_color == picture[y][x] && prev_color != 0){ // 이어져있음.
        areaSize[vIdx]++;
        searchedPixel[y][x] = true;
        cout<< vIdx << "  |  size : " << areaSize[vIdx] <<" : "<<x<<" "<<y<<endl;
    }
    else{                           // 현재 픽셀과는 이어지지 않음.
        if(picture[y][x]==0){
            prev_color = 0;
            searchedPixel[y][x] = true;
        }
